...
export function HeroCalculatorClient() {
  ...
  const contributionTerms = useMemo(() => result?.breakdown.map((row) => row.contribution.toFixed(2)) ?? [], [result]);
  ...
  return (
    <div className="relative mt-5 overflow-visible rounded-xl border border-emerald-900/10 bg-[#e8f5ef] p-5 sm:p-6" id="calculator">
      <div className="grid min-w-0 gap-6 lg:grid-cols-[minmax(0,1.35fr)_minmax(0,1fr)] lg:gap-8">
        <div className="min-w-0 space-y-5">
          <div className="rounded-lg border border-slate-200/80 bg-white/90 p-4">
            ...
            <ul className="mt-2 list-disc space-y-1.5 pl-5 text-base leading-relaxed text-[#0a0f1a]">
              <li>
                Enter a formula (<FormulaSub className="font-medium" formula="H2O" />) or a compound name (for example water
                or oxygen).
              </li>
              <li>
                While typing a name, matching compounds appear in a list under the field (scroll if the list is long). On
                phones and tablets, close it with the X or by choosing a compound. On desktop, it also closes when you click
            ...
            <div className="relative min-w-0 flex-1" ref={inputWrapRef}>
              ...
              {suggestionsOpen && suggestions.length > 0 ? (
                <div className="absolute left-0 right-0 top-full z-[60] mt-1 flex flex-col overflow-hidden rounded-md border border-slate-200 bg-white shadow-lg">
                  ...
                  <ul className="max-h-60 overflow-y-auto overscroll-contain py-1" id={SUGGESTIONS_LIST_ID} role="listbox">
                    {suggestions.map((row) => (
        ...
        <div className="min-w-0 space-y-4 border-t border-slate-200/80 pt-5 lg:border-l lg:border-slate-200/60 lg:border-t-0 lg:pl-8 lg:pt-0">
          {result ? (
            <>
              ...
              <div className="space-y-3">
                <div className="rounded-lg border border-slate-200/90 bg-white px-4 py-4 sm:px-5 sm:py-5">
                  <p className="min-[380px]:text-3xl break-words text-2xl font-bold tracking-tight text-[#0F766E] sm:whitespace-nowrap sm:text-4xl md:text-5xl">
            ...
            <p className="text-sm leading-relaxed text-[#0a0f1a]">
              Enter a valid formula or compound name (for example <FormulaSub formula="H2SO4" /> or{" "}
              <FormulaSub formula="Ca(OH)2" />
              ).
            </p>
          )}
        </div>
